...
La norme liste explicitement tous les points de codages requis. Pour le moment ils sont ici générés automatiquement :

```python
import unicodedata
from references.iso_cei_9995_11 import special_chars_from_dead_keys

latin_letters_with_diacritics = {}

for diacritic_on_circle in supported_diacritics:
	combining_diacritic = diacritic_on_circle[1]
	for letter in [l for cases in latin_letters for l in cases if l]:
		combination = (diacritic_on_circle, letter)

		# Add combinations defined in ISO/CEI 9995-11
		if combination in special_chars_from_dead_keys:
			latin_letters_with_diacritics[combination] = \
				special_chars_from_dead_keys[combination]

		# Add combinations that can be normalized in a single code point
		dst = letter + combining_diacritic
		dst = unicodedata.normalize("NFC", dst)
		if len(letter) == 1:
			latin_letters_with_diacritics[combination] = dst
```
