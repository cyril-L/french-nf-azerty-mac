...
Ce document n’est pas une référence, il contient des erreurs et des approximations (je n’ai pas encore pu le comparer au document original).
...
symbols = [
    ...
    #'∓', # U+2213 (MINUS-OR-PLUS SIGN) TODO check support
...
for combination, output in azerty_combinations.items():
    ...
    assert combined_with in supported_chars
...
for combination, output in azerty_combinations.items():
    dead_key, combined_with = combination
    assert dead_key in supported_by_azerty
    supported_by_azerty.add(output)

for char in supported_chars:
    assert char in supported_by_azerty
```
